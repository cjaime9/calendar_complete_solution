...
public class CustomerEventDao {
    ...
    public int removeCustomerFromEvent(long customerId, long eventId) {
        String sql = "DELETE  FROM customers_events WHERE customer_id = " +
                customerId + " AND  event_id = " + eventId + ");";
        return jdbcTemplate.update(sql);
    }

    public List<CustomerEvent> getCustomerEvents() {
        String sql = "SELECT events.id, events.event_name, events.start_time, events.end_time, " +
        ...
        List<CustomerEvent> customerEvents = jdbcTemplate.query(sql, new CustomerEventRowMapper());
        return customerEvents;
    }

    public List<CustomerEvent> getCustomerEventsForCustomer(int customerId) {
        String sql = "SELECT events.id, events.event_name, events.start_time, events.end_time, " +
                " customers.id, customers.first_name, customers.last_name from events" +
                " INNER JOIN customers_events on events.id = customers_events.event_id" +
                " INNER JOIN customers on customers.id = customers_events.customer_id" +
    ...
    public List<CustomerEvent> getAllCustomersForEvent(int eventId) {
        String sql = "SELECT events.id, events.event_name, events.start_time, events.end_time, " +
                " customers.id, customers.first_name, customers.last_name from events" +
                " INNER JOIN customers_events on events.id = customers_events.event_id" +
                " INNER JOIN customers on customers.id = customers_events.customer_id" +
